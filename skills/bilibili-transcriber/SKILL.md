...
- WorkBuddy：导入技能包后，用户可通过自然语言要求使用本技能。
- Codex 本地桌面或 CLI：用户可使用 `$bilibili-transcriber`，也可通过匹配描述的自然语言请求调用。使用相同脚本和数据目录，不另建转录实现。
- Codex 的文件、网络和命令权限仍由宿主控制；被沙箱拒绝时按宿主授权流程处理，不通过更换目录或命令绕过。安装 Skill 并不意味着已安装 Whisper 或已授权访问 Cookie。
- 后台任务需通过 status 查询完成；不要仅因 start 返回成功就宣称已转录完成，也不要承诺会话结束后 Agent 必定自动通知。
